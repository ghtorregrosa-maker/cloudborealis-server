...
INTENT_MAP: List[Tuple[List[str], str, str]] = [
    # Conversacion libre
    (["hola","buenos dias","buenas tardes","buenas noches","hey","que tal","como estas",
      "como andas","saludos","buen dia"], "chat", "saludo"),
    ...
    (["gracias","muchas gracias","genial","perfecto","excelente","buenisimo"], "chat", "agradecimiento"),
    # Aprendizaje - ampliado
    (["aprender sobre","aprende sobre","investiga sobre","aprende de","aprender de",
      "estudia sobre","busca sobre","aprende","aprender","aprende todo sobre",
...
def _parse_by_rules(text: str) -> Optional[Action]:
    ...
    # Detectar URL directa sin keyword
        ...
        for kw in keywords:
            ...
            if kw_n in text_n:
                ...
                if action_type == "chat":
                    ...
                    if subtype == "topic":
                        # Si hay URL en el texto, aprenderla
                        u = _extract_url(text)
                        if u: action.action_type="learn"; action.subtype="url"; action.target=u
                        else: action.target = _extract_quoted(text) or after
                    ...
                    if subtype == "navegar": action.target = _extract_url(text) or after
                    else: action.target = (_extract_quoted(text) or after).strip("'\"").strip()
...
def _parse_with_llm(text):
    ...
    try:
        ...
        msg = c.messages.create(
            ...
            system="Parser de comandos en espanol. Solo JSON: {action_type,subtype,target,parameters}. action_type: app,file,web,social,system,meta,learn,analyze,chat",
...
class Listener:
    ...
    def parse(self, text: str) -> Action:
        ...
        # Fallback: si tiene URL, aprender; si no, chat libre
        url = _extract_url(text)
        if url:
    ...
    def suggest_commands(self) -> List[str]:
        return [
            ...
            "aprender sobre techno melodico",
    ...
    def help_text(self) -> str:
        ...
        lines.append("")
        lines.append("  Tambien podes escribir en lenguaje libre:")
        lines.append("  * hola")
        lines.append("  * que podes hacer")
        lines.append("  * aprende FL studio  (sin comillas)")
        lines.append("  * aprende https://cualquier-url.com")
        return "\n".join(lines)
