# ================================================================
#  FIX_MIND.ps1
#  Reemplaza mind.py, verifica sintaxis, sube a GitHub,
#  espera el redeploy de Render y prueba las preguntas que fallaban.
#  EQM - El Que Manda | Borealis Corporations
# ================================================================
...
Clear-Host
Write-Host ""
Write-Host "  EQM - Fix mind.py + reasoner.py" -ForegroundColor Cyan
SEP
Write-Host ""

if (-not (Test-Path $DEPLOY)) {
    ERR "No se encontro $DEPLOY"
    Read-Host "Enter para salir"
    exit 1
}
OK "Carpeta de deploy encontrada."

$pythonExe = "F:\sistema completo\.venv\Scripts\python.exe"
if (-not (Test-Path $pythonExe)) { $pythonExe = "python" }

# ════════════════════════════════════════════════════════════
# ESCRIBIR mind.py CORREGIDO
# ════════════════════════════════════════════════════════════
INFO "Escribiendo mind.py corregido..."
...
Set-Content -Path "$DEPLOY\mind.py" -Value $mindLines -Encoding UTF8
OK "mind.py escrito ($($mindLines.Count) lineas)."

# ════════════════════════════════════════════════════════════
# VERIFICAR SINTAXIS
# ════════════════════════════════════════════════════════════
Set-Location $DEPLOY
INFO "Verificando sintaxis de mind.py..."

$syntaxCheck = & $pythonExe -c "import ast; ast.parse(open('mind.py', encoding='utf-8').read()); print('SYNTAX_OK')" 2>&1

if ($syntaxCheck -notmatch "SYNTAX_OK") {
    ERR "Error de sintaxis en mind.py:"
    Write-Host $syntaxCheck -ForegroundColor Red
    Read-Host "Enter para salir"
    exit 1
}
OK "mind.py verificado correctamente."

# Verificar tambien que reasoner.py exista y compile (no se toca, solo se verifica)
if (Test-Path "$DEPLOY\reasoner.py") {
    $reasonerCheck = & $pythonExe -c "import ast; ast.parse(open('reasoner.py', encoding='utf-8').read()); print('SYNTAX_OK')" 2>&1
    if ($reasonerCheck -match "SYNTAX_OK") {
        OK "reasoner.py existente verificado (sin cambios)."
    } else {
        ERR "reasoner.py tiene problemas de sintaxis - revisar manualmente."
    }
} else {
    ERR "ATENCION: no se encontro reasoner.py en $DEPLOY"
    ERR "mind.py necesita reasoner.py para funcionar. Verificar manualmente."
}
...
git commit -m "fix: conectar mind.py con reasoner.py - matematicas y filtro anti-coincidencia arbitraria" 2>&1 | Out-Null

INFO "Subiendo a GitHub..."
$pushResult = git push 2>&1

if ($LASTEXITCODE -eq 0) {
    OK "Codigo subido a GitHub exitosamente."
} else {
    ERR "Error en push:"
...
# ════════════════════════════════════════════════════════════
Write-Host ""
INFO "Esperando que Render redeploye (1-3 minutos)..."

$renderUrl = "https://cloudborealis-server.onrender.com"
$success   = $false

for ($i = 1; $i -le 24; $i++) {
    Start-Sleep -Seconds 8
    Write-Host "  Verificando... intento $i/24" -ForegroundColor DarkGray
    try {
        $health = Invoke-RestMethod "$renderUrl/api/health" -TimeoutSec 10 -ErrorAction Stop
        if ($health.status -eq "ok") { $success = $true; break }
    } catch {}
}

Write-Host ""
if ($success) {
    OK "Servidor respondiendo: $($health.app)"
} else {
    ERR "El servidor no respondio a tiempo. Puede seguir desplegando."
}

# ════════════════════════════════════════════════════════════
# PROBAR LAS PREGUNTAS QUE FALLABAN ANTES
# ════════════════════════════════════════════════════════════
Write-Host ""
SEP
Write-Host "  PROBANDO LAS PREGUNTAS QUE FALLABAN" -ForegroundColor Cyan
SEP
Write-Host ""

$headers = @{ "x-api-key" = "cloudborealis-secret-2025"; "Content-Type" = "application/json" }

$tests = @(
    "cuanto es 144 dividido 12",
...
foreach ($q in $tests) {
    INFO "Pregunta: '$q'"
    try {
        $body = @{ command = $q } | ConvertTo-Json
        $resp = Invoke-RestMethod -Uri "$renderUrl/api/command" -Method POST -Headers $headers -Body $body -TimeoutSec 30
        Write-Host "  EQM responde: $($resp.message)" -ForegroundColor Green
    } catch {
        Write-Host "  No se pudo probar: $_" -ForegroundColor Yellow
    }
    Write-Host ""
}
...
Write-Host "   - '144 dividido 12' -> 'El resultado es 12.'" -ForegroundColor DarkGray
Write-Host "   - 'fotosintesis'     -> definicion real aprendida" -ForegroundColor DarkGray
Write-Host "   - 'sdf'              -> NO debe devolver SDF-1 Macross" -ForegroundColor DarkGray
Write-Host ""
Write-Host "  Pagina: $renderUrl" -ForegroundColor White
Write-Host "  Dashboard: $renderUrl/dashboard" -ForegroundColor White
Write-Host ""
